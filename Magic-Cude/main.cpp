...
//必须得有这三个参数
{
	...
	case 'z':
		...
		roz[1][0][0] = (roz[1][0][0] + 90) % 360;
		roz[2][0][0] = (roz[2][0][0] + 90) % 360;

		roz[0][1][0] = (roz[0][1][0] + 90) % 360;
		roz[1][1][0] = (roz[1][1][0] + 90) % 360;
		roz[2][1][0] = (roz[2][1][0] + 90) % 360;

		roz[0][2][0] = (roz[0][2][0] + 90) % 360;
		roz[1][2][0] = (roz[1][2][0] + 90) % 360;
		roz[2][2][0] = (roz[2][2][0] + 90) % 360;
		glutPostRedisplay();
		break;
	case 'r':
		for (int i = 0; i < 3; i++)
